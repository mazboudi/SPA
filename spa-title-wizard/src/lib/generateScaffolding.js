...
`;
    ...
    if (isPassthrough && refactorScript) {
      ...
      // Form-Sync or Customized mode: ALWAYS generate and commit the .ps1 script
      const psadtScript = s.isCustomized
        ? (s.customScriptContent || generatePsadtScript(s))
        : generatePsadtScript(s);
